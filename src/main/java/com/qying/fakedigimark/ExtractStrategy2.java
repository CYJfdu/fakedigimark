...
public class ExtractStrategy2 {
    ...
    public static void extract(String filename){
        ...
        for(int i=0;i<blank.rows()/32;i++){
            for(int j=0;j<blank.cols()/32;j++) {
                if(queryDCT(blank,i*32,j*32,map)) {
                    for(int k=0;k<8;k++){
                        for(int l=0;l<8;l++){
                            blank.put(i*32+k,j*32+l,0);
                        }
                    }
                    System.out.println("Embedded in: " + i + " " + j + " ");
                }

            }
        }

        HighGui.imshow("展示图片", blank);
        HighGui.waitKey();
...
//        System.out.println("Target File: "+filename);
    ...
    public static boolean queryDCT(Mat blank, int xbegin, int ybegin, TreeMap<Double,int[]> map){
        //嵌入4个连续的block
//        int i=0,j=0;
...
//            System.out.println();
//        }
        double sum1 = 0;int num = 0;
//        int[] x = Settings.x;int[] y = Settings.y;
        for (int i = 0; i < 32; i++) {
            for (int j = 0; j < 32; j++) {
                if(i+j>32-4 && i+j<32+4) {
                    sum1 += Math.abs(block.get(i, j)[0]);
                    num++;
                }
            }
        ...
        return sum1/num>Settings.strength/2;
